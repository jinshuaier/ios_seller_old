...
@interface TCMeshdViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation TCMeshdViewController
...
//    UIBarButtonItem *right = [[UIBarButtonItem alloc]initWithTitle:@"全部已读" style:UIBarButtonItemStyleDone target:self action:@selector(biaoji)];
...
- (void)request{
    ...
    [manager POST:[TCServerSecret loginAndRegisterSecret:@"600008"] parameters:paramter progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    NSDictionary *paramter = @{@"id":[_userdefaults valueForKey:@"userID"], @"token":[_userdefaults valueForKey:@"userToken"], @"mid":_messageArr[indexPath.section][@"id"], @"type":@"2"};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_messageArr.count != 0) {
        ...
        if ([_messageArr[indexPath.section][@"status"] isEqualToString:@"0"]) {
            if ([_messageArr containsObject: _messageArr[indexPath.section][@"id"]]) {
...
- (void)barButtonItemPressed:(UIButton *)sender{
    ...
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    [manager POST:[TCServerSecret loginAndRegisterSecret:@"600009"] parameters:@{@"id":[_userdefaults valueForKey:@"userID"], @"token":[_userdefaults valueForKey:@"userToken"], @"menu":@"2"} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
