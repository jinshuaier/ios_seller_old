...
#import "TCOrderSellViewController.h"
#import "TCChooseSlideView.h"
#import "TCOrderSellTableViewCell.h"
#import "TCOrderNumModel.h"

@interface TCOrderSellViewController ()<UITableViewDelegate,UITableViewDataSource,TCChooseSlideProtocol>
...
@implementation TCOrderSellViewController
...
- (void)createUI
{
    ...
    //订单数的标题
    self.orderTitleLabel = [UILabel publicLab:@"总销售额（元）" textColor:TCUIColorFromRGB(0xFFFFFF) textAlignment:NSTextAlignmentLeft fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
    self.orderTitleLabel.frame = CGRectMake(15, 14, WIDHT - 15, 13);
    [headView addSubview:self.orderTitleLabel];
    
    //订单的数值
    self.orderNumLabel = [UILabel publicLab:@"0" textColor:TCUIColorFromRGB(0xFFFFFF) textAlignment:NSTextAlignmentLeft fontWithName:@"PingFangSC-Semibold" size:36 numberOfLines:0];
    ...
    NSArray *menuArray = [NSArray arrayWithObjects:@"总销售额",@"本月销售额", @"今日销售额",nil];
...
- (void)request:(NSString *)statusx{
    ...
    [TCNetworking postWithTcUrlString:[TCServerSecret loginAndRegisterSecretOffline:@"202003"] paramter:dicc success:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if (jsonDic[@"data"]) {
            ...
            self.orderNumLabel.text = [NSString stringWithFormat:@"%@",jsonDic[@"data"][@"totalMoney"]];
        } else {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TCOrderSellTableViewCell *cell = [[TCOrderSellTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
...
-(void)_getTag:(NSInteger)tag
{
    if (tag == 0){
        [self setupRefresh:@"1"];
        self.orderTitleLabel.text = @"总销售额";
    }else if (tag == 1){
        [self setupRefresh:@"2"];
        self.orderTitleLabel.text = @"本月销售额";
        
    } else {
        [self setupRefresh:@"3"];
        self.orderTitleLabel.text = @"今日销售额";
