...
#import "TCOrderNumViewController.h"
#import "TCChooseSlideView.h"
#import "TCOrderNumTableViewCell.h"
#import "TCOrderDetailViewController.h"

@interface TCOrderNumViewController ()<UITableViewDelegate,UITableViewDataSource,TCChooseSlideProtocol>
...
@implementation TCOrderNumViewController
...
- (void)createUI
{
    ...
    //订单数的标题
    self.orderTitleLabel = [UILabel publicLab:@"总订单数" textColor:TCUIColorFromRGB(0xFFFFFF) textAlignment:NSTextAlignmentLeft fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
    ...
    NSArray *menuArray = [NSArray arrayWithObjects:@"总订单数",@"本月订单数", @"今日订单数",nil];
    ...
    [TCNetworking postWithTcUrlString:[TCServerSecret loginAndRegisterSecretOffline:@"202003"] paramter:dicc success:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if (jsonDic[@"data"]) {
            ...
            for (NSDictionary *dic in arr) {
                TCOrderNumModel *model = [TCOrderNumModel orderNumInfoWithDictionary:dic];
...
- (void)requestUp:(int)page andStatus:(NSString *)statusx{
    ...
    NSDictionary *parameters = @{@"page":@(page),@"typeKey":statusx,@"shopId":shopID,@"sign":singStr};
    NSDictionary *dicc = [TCServerSecret report:parameters];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TCOrderNumTableViewCell *cell = [[TCOrderNumTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    if (self.dataArr.count == 0){
        
    } else {
        TCOrderNumModel *model = self.dataArr[indexPath.row];
        cell.model = model;
    }
...
{
    TCOrderNumModel *model = self.dataArr[indexPath.row];
    TCOrderDetailViewController *orderDetiVC = [[TCOrderDetailViewController alloc] init];
    orderDetiVC.oid = model.orderid;
    orderDetiVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:orderDetiVC animated:YES];
    orderDetiVC.hidesBottomBarWhenPushed = NO;
}
...
-(void)_getTag:(NSInteger)tag
{
    if (tag == 0){
        [self setupRefresh:@"1"];
        self.orderTitleLabel.text = @"总订单数";
    }else if (tag == 1){
        [self setupRefresh:@"2"];
        self.orderTitleLabel.text = @"本月订单数";
    } else {
        [self setupRefresh:@"3"];
        self.orderTitleLabel.text = @"今日订单数";
    }
}
