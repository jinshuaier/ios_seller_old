...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        if ([shopStr isEqualToString:@"0"]){
            ...
            [alert addAction:[UIAlertAction actionWithTitle:@"完善店铺" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            [alert addAction:[UIAlertAction actionWithTitle:@"暂不完善" style:UIAlertActionStyleCancel handler:nil]];
...
//商品销量榜 按钮点击
- (void)clickhot{
    TCSellCountViewController *sellcount = [[TCSellCountViewController alloc]init];
    sellcount.shopid = _idStr;
    self.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:sellcount animated:YES];
    self.hidesBottomBarWhenPushed = NO;
}
