...
-(void)createView
{
    ...
    proLabel.text = @"为了更好的为您服务，输入您宝贵的建议和意见，我们将不断的为您改进。";
    proLabel.numberOfLines = 0;
    proLabel.textColor = SmallTitleColor;
    proLabel.font = [UIFont systemFontOfSize:14*HEIGHTSCALE];
    ...
    self.contentView.font = [UIFont systemFontOfSize:14.0f];
...
-(void)createQuest
{
    ...
    [TCAFNetworking postWithURLString:[TCServerSecret loginAndRegisterSecret:@"200016"] parameters:dic success:^(id responseObject) {
        ...
        if (!([[dic valueForKey:@"retValue"]integerValue] == 5)) {
