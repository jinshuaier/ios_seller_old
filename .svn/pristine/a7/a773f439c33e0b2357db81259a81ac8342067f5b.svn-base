...
//
//
//@interface TCSellViewController ()<UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate>
...
//@property (nonatomic, strong) NSString *idnum;//用来记录请求的分类id
//@property (nonatomic, assign) BOOL isCanRefresh;//判断是否可以刷新
//@end
//
//
//@implementation TCSellViewController
//
...
//    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(thing:) name:@"shuaxinsort" object:nil];
//}
//
//- (void)thing:(NSNotification *)not{
//    if ([not.userInfo[@"yemian"] isEqualToString:@"0"]) {
...
//            [_xiajia setTitle:@"下架" forState:UIControlStateNormal];
//        }else{
...
//    NSLog(@"%@  %@  %@", [_userdefaults valueForKey:@"userID"], [_userdefaults valueForKey:@"userToken"], _shopid);
//    [TCNetworking postWithTcUrlString:[TCServerSecret loginAndRegisterSecret:@"300101"] paramter:@{@"id":[_userdefaults valueForKey:@"userID"], @"token":[_userdefaults valueForKey:@"userToken"], @"shopid":_shopid, @"status":@"1"} success:^(NSString *jsonStr, NSDictionary *jsonDic) {
//        if (jsonDic[@"data"]) {
//            //存储一级分类
//            [_firstSortArr addObjectsFromArray:jsonDic[@"data"][@"cat_list"]];
...
//            //如果有中文
//            NSString *string1 = _shopsMuArr[indexPath.row][@"headPic"];
...
//
//        //滑动之前判断当前rview位置  如果在1/4职位（原始位置） 只在为负值的时候发通知 变形   即只能向左滑动
