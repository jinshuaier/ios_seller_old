...
#define PWD_COUNT 6
#define DOT_WIDTH 10
@interface TCinputPViewController ()<UITextFieldDelegate,UIGestureRecognizerDelegate>
{
...
@implementation TCinputPViewController
...
-(void)creatUI{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(12, 48, 160, 21)];
    label.text = @"请输入六位密码";
    ...
    _inputView = [[UIView alloc]initWithFrame:CGRectMake(24, CGRectGetMaxY(label.frame) + 16, WIDHT - 48, 52)];
    ...
    UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(WIDHT - 24 - 48, CGRectGetMaxY(_inputView.frame) + 10, 48, 17)];
...
-(void)clickForget:(UIButton *)sender{
    NSLog(@"忘记密码");
    if ([self.entranceTypeStr isEqualToString:@"2"]) {
        TCSetpwdViewController *setVC = [[TCSetpwdViewController alloc]init];
        setVC.titleStr = @"修改支付密码";
        ...
        TCSetpwdViewController *setVC = [[TCSetpwdViewController alloc]init];
        setVC.titleStr = @"修改支付密码";
        setVC.entranceTypeStr = @"3";
        setVC.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:setVC animated:YES];
    }else if ([self.entranceTypeStr isEqualToString:@"4"]){
        TCSetpwdViewController *setVC = [[TCSetpwdViewController alloc]init];
        setVC.titleStr = @"修改支付密码";
        setVC.entranceTypeStr = @"4";
        setVC.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:setVC animated:YES];
    }
}
...
-(void)creatRequest{
    ...
    NSDictionary *dic = @{@"mid":midStr,@"token":tokenStr,@"timestamp":Timestr,@"payCode":self.pwdStr};
    NSString *singStr = [TCServerSecret signStr:dic];
    NSDictionary *parameters = @{@"sign":singStr,@"mid":midStr,@"token":tokenStr,@"timestamp":Timestr,@"payCode":self.pwdStr};
    [TCNetworking postWithTcUrlString:[TCServerSecret loginAndRegisterSecretOffline:@"201031"] paramter:parameters success:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if ([codeStr isEqualToString:@"1"]) {
            ...
            TCAddBankCardViewController *addBankVC = [[TCAddBankCardViewController alloc]init];
            addBankVC.entranceTypeStr = self.entranceTypeStr;
